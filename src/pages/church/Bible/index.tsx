...
export default function Bible() {
  ...
  const handleFetchVerse = useCallback((values: BibleSearchFormValues) => {
    ...
    if (values.verse === "") {
      apiUrl = `https://bible-api.com/${values.book}+${values.chapter}?translation=${t(`churchPages.bible.language.${i18n.language}`)}`;
    } else {
      apiUrl = `https://bible-api.com/${values.book}+${values.chapter}:${values.verse}?translation=${t(`churchPages.bible.language.${i18n.language}`)}`;
    }
    ...
    fetch(apiUrl)
      ...
      .catch(() => {
        setVerseText(`${t("churchPages.bible.fetch.error")}`);
      });
  }, []);
      ...
      >
        {({ handleSubmit }) => (
          <ScrollView style={styles.container}>
            <SelectComponent name="book" label={t("churchPages.bible.book")} items={bookOptions} />

            <SelectComponent name="chapter" label={t("churchPages.bible.chapter")} items={chapterOptions} />

            <SelectComponent name="verse" label={t("churchPages.bible.verse")} items={verseOptions} />

            <TouchableOpacity style={styles.button} onPress={() => handleSubmit()}>
              <Text style={styles.buttonText}>{t("churchPages.bible.search")}</Text>
            </TouchableOpacity>
            ...
            {verseText ? (
              <>
                <Text style={styles.information}>{t("churchPages.bible.result")}</Text>
              ...
              <>
                ...
                <Text style={styles.noSearch}>{t("churchPages.bible.noSearch")}</Text>
              </>
            )}
          </ScrollView>
        )}
